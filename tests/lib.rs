...
fn nlmsg_put_check() {
    ...
    {
        let nlh = mnl::Nlmsg::new(&mut buf);
        assert!(nlh.put_check(512, 123, &std::u64::MAX));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN(attr_len as u32));
        ...
        assert!(!nlh.put_check(39, 234, &std::u64::MAX));
        assert!(nlh.put_check(40, 234, &std::u64::MAX));
    }
    assert!(*buf_offset_as::<u16>(&buf, 16) == attr_len);
    assert!(*buf_offset_as::<u16>(&buf, 18) == 123);
    assert!(*buf_offset_as::<u64>(&buf, 20) == std::u64::MAX);
...
fn nlmsg_put_u8_check() {
    ...
    {
        let nlh = mnl::Nlmsg::new(&mut buf);
        assert!(nlh.put_u8_check(512, 12, 34));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN(attr_len as u32));
...

    {
        let nlh = mnl::Nlmsg::from_bytes(&mut buf);
        let attr = nlh.payload_tail::<linux::netlink::Nlattr>();
        assert!(!nlh.put_u8_check(31, 56, 78));
        assert!(nlh.put_u8_check(32, 56, 78));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN((attr_len) as u32) * 2);
...
fn nlmsg_put_u16_check() {
    ...
    {
        let nlh = mnl::Nlmsg::new(&mut buf);
        assert!(nlh.put_u16_check(512, 1234, 5678));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN(attr_len as u32));
...

    {
        let nlh = mnl::Nlmsg::from_bytes(&mut buf);
        let attr = nlh.payload_tail::<linux::netlink::Nlattr>();
        assert!(!nlh.put_u16_check(31, 9012, 3456));
        assert!(nlh.put_u16_check(32, 9012, 3456));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN((attr_len) as u32) * 2);
...
fn nlmsg_put_u32_check() {
    ...
    {
        let nlh = mnl::Nlmsg::new(&mut buf);
        assert!(nlh.put_u32_check(512, 1234, 56789012));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN(attr_len as u32));
...

    {
        let nlh = mnl::Nlmsg::from_bytes(&mut buf);
        let attr = nlh.payload_tail::<linux::netlink::Nlattr>();
        assert!(!nlh.put_u32_check(31, 3456, 78901234));
        assert!(nlh.put_u32_check(32, 3456, 78901234));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN((attr_len) as u32) * 2);
...
fn nlmsg_put_u64_check() {
    ...
    {
        let nlh = mnl::Nlmsg::new(&mut buf);
        assert!(nlh.put_u64_check(512, 1234, 0x567890abcdef0123));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN(attr_len as u32));
...

    {
        let nlh = mnl::Nlmsg::from_bytes(&mut buf);
        let attr = nlh.payload_tail::<linux::netlink::Nlattr>();
        assert!(!nlh.put_u64_check(39, 4567, 0x890abcdef0123456));
        assert!(nlh.put_u64_check(40, 4567, 0x890abcdef0123456));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN((attr_len) as u32) * 2);
...
fn nlmsg_put_str_check() {
    ...
    {
        let nlh = mnl::Nlmsg::new(&mut buf);
        assert!(nlh.put_str_check(512, 1234, s1));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN() + mnl::ALIGN(attr_len1 as u32));
    ...
    let bi: isize;
    {
        ...
        let attr = nlh.payload_tail::<linux::netlink::Nlattr>();
        bi = nlh.nlmsg_len as isize;
        assert!(!nlh.put_str_check(51, 5678, s2));
        assert!(nlh.put_str_check(52, 5678, s2));
        assert!(nlh.nlmsg_len == mnl::NLMSG_HDRLEN()
                + mnl::ALIGN(attr_len1 as u32) + mnl::ALIGN(attr_len2 as u32));
...
fn nlmsg_put_strz_check() {
    ...
    {
        let nlh = mnl::Nlmsg::new(&mut buf);
        assert!(nlh.put_strz_check(512, 1234, s1));
        assert!(nlh.nlmsg_len == nlmsg_len);
    ...
    let bi: isize;
    {
        ...
        let attr = nlh.payload_tail::<linux::netlink::Nlattr>();
        bi = nlh.nlmsg_len as isize;
        assert!(!nlh.put_strz_check(51, 5678, s2));
        assert!(nlh.put_strz_check(52, 5678, s2));
        assert!(nlh.nlmsg_len == nlmsg_len);
...
fn nlmsg_nest_start_check() {
    ...
    {
        let nlh = mnl::Nlmsg::new(&mut buf);
        assert!(nlh.nest_start_check(19, 0x123).is_none());
        assert!(nlh.nlmsg_len == linux::netlink::NLMSG_HDRLEN());
        if let Some(attr) = nlh.nest_start_check(20, 0x123) {
            assert!(nlh.nlmsg_len == linux::netlink::NLMSG_LENGTH(linux::netlink::NLA_HDRLEN() as u32));
            assert!(attr.nla_len == 0); // will update after _end
            assert!(attr.nla_type & linux::netlink::NLA_F_NESTED != 0);
            assert!(attr.nla_type & linux::netlink::NLA_TYPE_MASK == 0x123);

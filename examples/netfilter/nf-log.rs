...
fn log_cb<T>(nlh: &mnl::Nlmsg, _: &mut T) -> mnl::CbRet {
    ...
    match tb[nful::AttrType::PACKET_HDR as usize] {
        Some(attr) => ph = attr.payload(),
        None => {},
    }
    match tb[nful::AttrType::PREFIX as usize] {
        Some(attr) => prefix = attr.str(),
        None => {},
    }
    match tb[nful::AttrType::MARK as usize] {
        Some(attr) => mark = attr.u32(),
        None => {},
    }
...
// fn nflog_build_cfg_pf_request(buf: &mut [u8], command: u8) -> &mut mnl::Nlmsg {
